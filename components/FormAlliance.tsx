...
const FormAlliance = () => {
  ...
  const handleSimulate = () => {
    ...
    // ---- Data suggerida basada en calendari ----
    const today = new Date();
    const assignedUser = formData.assignedTo;

    // Filter user’s active projects
    const userProjects = data.filter(p => p.assignedTo === assignedUser && !p.completed);

    let accumulatedHours = 0;
    userProjects.forEach(proj => {
      accumulatedHours += Number(proj.totalTime || 0);
    });
    accumulatedHours += baseTime;

    const daysToAdd = Math.ceil(accumulatedHours / 8);
    ...
    setSimulation(prev => ({
      ...
      suggestedDate: nextAvailableDate.toISOString().split('T')[0]
    }));

    setFormData(prev => ({
      ...prev,
      agreedPrice: Math.round(price).toString(),
      deadline: nextAvailableDate.toISOString().split('T')[0]
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setFormData({
      projectName: '', stoneSize: '', stoneType: '', material: '', style: '',
      shape: '', timePerStone: '', pricePerStone: '', goldWeight: '', client: '', stoneCount: '', deadline: '', assignedTo: '', agreedPrice: ''
    });
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-10">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Form */}
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="lg:col-span-2 bg-white rounded-2xl shadow-lg border border-amber-100 overflow-hidden">
        ...
        {/* Simulator Panel */}
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="bg-white rounded-2xl shadow-xl overflow-hidden flex flex-col border border-jewelry-gold/30">
          ...
          <div className="p-6 space-y-6 flex-1 bg-white">
            <div>
              ...
              <input type="range" min="1" max="2" step="0.1" value={simulation.difficultyMargin} onChange={e => setSimulation({...simulation, difficultyMargin: parseFloat(e.target.value)})} className="w-full h-2 bg-amber-100 rounded-lg appearance-none cursor-pointer accent-jewelry-gold" />
          ...
          <div className="p-6 bg-amber-50/50 border-t border-amber-100">
            ...
            <div className="relative mb-4">
              ...
              <input name="agreedPrice" value={formData.agreedPrice} onChange={handleChange} className="w-full pl-12 p-3 bg-white border border-jewelry-gold/30 rounded-lg text-xl font-serif font-bold text-gray-800 shadow-sm focus:ring-2 focus:ring-jewelry-gold outline-none" placeholder="0.00" />
            </div>
            <button form="projectForm" type="submit" disabled={submitting} className="w-full bg-jewelry-copper hover:bg-jewelry-bronze text-white py-3 rounded-lg font-bold shadow-lg transition disabled:opacity-50 flex justify-center">

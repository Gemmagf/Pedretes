...
const shapeList = ['rund','oval','princess','emerald','marquise','pear','heart','cushion'];
const materialList = ['WG', 'GG', 'Roségold', 'Platin'];
const styleList = ['Classic','Modern','Vintage','Micro-Pavé','Halo'];
const stoneTypes = ['Diamant','Saphir','Rubin','Smaragd','Sonstige'];
...
const FormFassung = () => {
  ...
  const refreshData = async () => {
    setLoading(true);
    const res = await getProjects('Fassung');
    setData(res);
    setLoading(false);
  };
  ...
  const handleSimulate = () => {
    ...
    const daysToAdd = Math.ceil((baseTime / 60) / 6);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setFormData({
      projectName: '', client: '', assignedTo: '', shape: '', material: '',
      style: '', stoneType: '', stoneCount: '', totalTime: '', pricePerStone: '',
      goldWeight: '', deadline: '', status: 'Pending', agreedPrice: ''
    });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        >
          ...
          <form id="fassungForm" onSubmit={handleSubmit} className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {[
              { name: 'client', label: 'Kunde', type: 'select', opts: clientsList },
              { name: 'assignedTo', label: t('assignedTo'), type: 'select', opts: users.map(u => ({ val: u.id, text: u.name })) },
              { name: 'shape', label: 'Steinform', type: 'select', opts: shapeList },
              { name: 'material', label: 'Material', type: 'select', opts: materialList },
              { name: 'style', label: 'Stil', type: 'select', opts: styleList },
              { name: 'stoneType', label: 'Steinart', type: 'select', opts: stoneTypes },
              { name: 'stoneCount', label: 'Anzahl Steine', type: 'number' },
              { name: 'totalTime', label: 'Total Zeit (Minuten)', type: 'number' },
              { name: 'pricePerStone', label: 'Preis pro Stein (CHF)', type: 'number' },
              { name: 'goldWeight', label: 'Gold zurück (g)', type: 'number', step: '0.1' },
              { name: 'deadline', label: t('deadline'), type: 'date' },
              { name: 'status', label: t('status'), type: 'select', opts: ['Pending', 'In Progress', 'Completed'] },
            ].map(field => (
              <div key={field.name}>
                ...
                {field.type === 'select' ? (
                  <select
                    ...
                    className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-jewelry-gold focus:bg-white outline-none transition cursor-pointer"
                  >
                  ...
                  <input
                    ...
                    value={(formData as any)[field.name]}
                    onChange={handleChange}
                    className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-jewelry-gold focus:bg-white outline-none transition"
                  />
                )}
              </div>
            ))}

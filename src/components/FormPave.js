...
const FormPave = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const json = await getSheetData('Pave_Form');
  ...
  const handleSubmit = async e => {
    ...
    const dataToSend = { ...formData, id: uuidv4(), sheet: 'Pave_Form' };

    try {
      const res = await addRow('Pave_Form', dataToSend);
      if (res.status === 'success') {
        setSubmitted(true);
        setFormData({
          projectName: '', clientName: '', description: '', dueDate: '',
          gemsQuantity: '', color: '', timePerStone: '', pricePerStone: '', optimizePrice: false
        });
        fetchData();
  ...
  return (
    <motion.div className="bg-white/80 p-6 rounded-xl shadow max-w-5xl mx-auto">
      <h2 className="text-2xl font-semibold mb-4">{t('paveFormTitle')}</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-3 mb-6">
        ...
        <div>
          ...
          <input type="number" name="gemsQuantity" value={formData.gemsQuantity} onChange={handleChange} className="w-full border rounded px-2 py-1" />
        </div>
